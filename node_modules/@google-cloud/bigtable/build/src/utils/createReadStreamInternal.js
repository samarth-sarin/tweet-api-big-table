...
exports.createReadStreamInternal = createReadStreamInternal;
exports.isRstStreamError = isRstStreamError;
const is = require("is");
...
const timed_stream_1 = require("../timed-stream");
/**
 ...
 *
 */
function createReadStreamInternal(table, metricsCollector, opts) {
    const options = opts || {};
     ...
     */
    const timeout = opts?.gaxOptions?.timeout ||
        (table?.bigtable?.options?.BigtableClient?.clientConfig?.interfaces &&
            table?.bigtable?.options?.BigtableClient?.clientConfig?.interfaces['google.bigtable.v2.Bigtable']?.methods['ReadRows']?.timeout_millis);
    ...
    let lastRowKey = '';
    let rowsRead = 0;
    const userStream = new timed_stream_1.TimedStream({
        transformHook(event, _encoding, callback) {
            if (userCanceled) {
                callback();
                return;
            }
            if (event.eventType === chunktransformer_1.DataEvent.LAST_ROW_KEY_UPDATE) {
                /**
    ...
    const makeNewRequest = () => {
        ...
        retryTimer = null;
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        chunkTransformer = new chunktransformer_1.ChunkTransformer({
        ...
        const gaxOpts = (0, tabular_api_surface_1.populateAttemptHeader)(numRequestsMade, options.gaxOptions);
        ...
        const toRowStream = new stream_1.Transform({
            transform: (rowData, _, next) => {
                ...
                if (rowData.eventType ===
                    chunktransformer_1.DataEvent.LAST_ROW_KEY_UPDATE) {
                    /**
                     ...
                     */
                    const row = table.row(rowData.key);
        ...
        rowStream
            ...
            if (tabular_api_surface_1.IGNORED_STATUS_CODES.has(error.code)) {
                // We ignore the `cancelled` "error", since we are the ones who cause
                // it when the user calls `.abort()`.
                userStream.end();
                metricsCollector.onOperationComplete(error.code, userStream.getTotalDurationMs());
                return;
            }
            numConsecutiveErrors++;
            numRequestsMade++;
            if (numConsecutiveErrors <= maxRetries &&
                (tabular_api_surface_1.RETRYABLE_STATUS_CODES.has(error.code) || isRstStreamError(error)) &&
                !(timeout && timeout < new Date().getTime() - callTimeMillis)) {
                const backOffSettings = options.gaxOptions?.retry?.backoffSettings ||
                    tabular_api_surface_1.DEFAULT_BACKOFF_SETTINGS;
                const nextRetryDelay = (0, tabular_api_surface_1.getNextDelay)(numConsecutiveErrors, backOffSettings);
                ...
                if (!error.code &&
                    ...
                    //
                    error.code = google_gax_1.grpc.status.CANCELLED;
                }
                metricsCollector.onOperationComplete(error.code, userStream.getTotalDurationMs());
            ...
            .on('end', () => {
            activeRequestStream = null;
            metricsCollector.onOperationComplete(google_gax_1.grpc.status.OK, userStream.getTotalDurationMs());
        });
...
function isRstStreamError(error) {
...
//# sourceMappingURL=createReadStreamInternal.js.map